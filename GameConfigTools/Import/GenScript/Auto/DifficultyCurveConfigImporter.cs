...
if (!VaildUtil.TryConvert(values[i][1], out difficultyMin,int.MinValue,int.MaxValue))
{
	errMsg = string.Format("{8} {0}.xlsx sheet:[{1}] [{2},{3}]读取出现错误，{4}必须为{5} - {6} {7}型", this.GetConfigName(), sheetName, row,1+1, difficultyMin,int.MinValue,int.MaxValue,"int","difficultyMin");
	return;
}
if (!VaildUtil.TryConvert(values[i][2], out difficultyMax,int.MinValue,int.MaxValue))
{
	errMsg = string.Format("{8} {0}.xlsx sheet:[{1}] [{2},{3}]读取出现错误，{4}必须为{5} - {6} {7}型", this.GetConfigName(), sheetName, row,2+1, difficultyMax,int.MinValue,int.MaxValue,"int","difficultyMax");
	return;
}
