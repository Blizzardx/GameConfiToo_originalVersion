...
if (!VaildUtil.TryConvert(values[i][1], out RgbaValue,string.Empty,string.Empty))
{
	errMsg = string.Format("{8} {0}.xlsx sheet:[{1}] [{2},{3}]读取出现错误，{4}必须为{5} - {6} {7}型", this.GetConfigName(), sheetName, row,1+1, RgbaValue,string.Empty,string.Empty,"string","RgbaValue");
	return;
}
